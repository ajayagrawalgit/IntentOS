...
EMERGENCY_EMAIL_TO = os.getenv("EMERGENCY_EMAIL_TO") or "ajayagrawalhere@gmail.com"

def send_emergency_email(user_info: dict, location_data: dict, analysis: dict, emergency_emails: str = None):
    """
    Sends high-priority emergency emails to the specified list of recipients.
    """
    ...
    if emergency_emails:
        ...
        recipients = [email.strip() for email in emergency_emails.split(",") if email.strip()]
    
    if not recipients and EMERGENCY_EMAIL_TO:
        recipients = [EMERGENCY_EMAIL_TO]

    if not recipients:
        logger.error("No recipients found for emergency email.")
        return False
    ...
    for recipient in recipients:
        try:
            ...
            logger.error(f"Failed to send emergency email to {recipient}: {e}")
            success = False
